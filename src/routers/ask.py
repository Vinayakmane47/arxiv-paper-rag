    ...
    with rag_tracer.trace_request("api_user", request.query) as trace:
        try:
            ...
            if cache_client:
                try:
                    ...
                    if cached_response:
                        ...
                        # Update trace with cached response before returning
                        total_duration = time.time() - start_time
                        rag_tracer.end_request(
                            trace, 
                            cached_response.answer, 
                            total_duration,
                            metadata={"cache_hit": True, "sources": cached_response.sources, "chunks_used": cached_response.chunks_used}
                        )
            ...
            if not chunks:
                ...
                rag_tracer.end_request(trace, response.answer, time.time() - start_time, metadata={"cache_hit": False, "no_chunks": True})
                return response
            ...
            rag_tracer.end_request(trace, answer, time.time() - start_time, metadata={"cache_hit": False, "sources": sources, "chunks_used": len(chunks)})
    ...
    async def generate_stream():
        ...
        with rag_tracer.trace_request("api_user", request.query) as trace:
            try:
                ...
                if cache_client:
                    try:
                        ...
                        if cached_response:
                            ...
                            # Update trace with cached response
                            total_duration = time.time() - start_time
                            rag_tracer.end_request(
                                trace,
                                cached_response.answer,
                ...
                rag_tracer.end_request(trace, full_response, time.time() - start_time, metadata={"cache_hit": False, "sources": sources, "chunks_used": len(chunks)})
